    ...
    <main class="content">
        ...
        <div class="pagination" v-if="(!loading && post_count > 0)">
          ...
          <ul v-if="( page_count  > 1)" class="posts-pagination list-inline">
            <li :class="[{ active: (paged_index == page_index) }]"   v-for="page_index in page_count" :key="page_index"><router-link :to="{ name: 'Blog-Paged', params: { paged_index: page_index }}">{{ page_index }}</router-link></li>
          </ul>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            getPosts: function() {
                const wpPromisedResult = WordpressService.getPosts( this.paged_index, Config.posts_per_page );
